...
PARA = re.compile(r"^(?P<name>.+?) didn'?t visit (?P<place>.+?)$")
TEMPLATES = {
...
DEMO_VARIANT = {"name": "orig"}


def rephrase(hypothesis):
    """Put a demo hypothesis into the same surface form as the test items."""
    m = PARA.match(hypothesis)
    if not m:
        return hypothesis
    return TEMPLATES[DEMO_VARIANT["name"]].format(**m.groupdict())


def variant_of(item, condition):
    """Return (hypothesis_text, correct_label) for this condition."""
    if condition == "negated":
        h = item.get("variant") and item["hypothesis"] or rephrase(item["hypothesis"])
        return h, item["gold_label"]
...
def main():
    ...
    if items and items[0].get("variant"):
        DEMO_VARIANT["name"] = items[0]["variant"]
    print("demos rephrased to variant:", DEMO_VARIANT["name"])
